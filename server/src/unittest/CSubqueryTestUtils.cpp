	...
	)
{
	...
	CColRef *pcrComputed = pcf->PcrCreate(pmdtype, popSubquery->PmdidCollation(), popSubquery->ITypeModifier());
	...
	)
{
	...
	pcrComputed = pcf->PcrCreate(pcr->Pmdtype(), pcr->PmdidCollation(), pcr->ITypeModifier());
	...
	CExpression *pexprSubqueryAll = PexprSubqueryQuantified(pmp, COperator::EopScalarSubqueryAll, pexprOuter, pexprGet, fCorrelated);
	pcrComputed = pcf->PcrCreate(pmdtypebool, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier);
	...
	)
{
	...
	CColRef *pcrComputed = pcf->PcrCreate(pmdtype, pop->PmdidCollation(), pop->ITypeModifier());
