...
CExpressionTest::EresUnittest_BitmapGet()
{
	...
	CColRef *pcrFirst = pcf->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), pmdcol->ITypeModifier());
	...
	for (ULONG ul = 0; ul < ulCols; ++ul)
	{
		...
		const IMDType *pmdtype = mda.Pmdtype(pmdcol->PmdidType());
		CColRef *pcr = pcf->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), pmdcol->ITypeModifier());
...
CExpressionTest::EresUnittest_FValidPlan()
{
	...
	// Test that the plan is NOT valid.
	{
		...
		CColRef *pcrComputed = pcf->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier);
