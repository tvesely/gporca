...
CEquivalenceClassesTest::EresUnittest_NotDisjointEquivalanceClasses()
{
	...
	{
		CColRef *pcr = pcf->PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name);
		pcrs->Include(pcr);
	...
	CColRef *pcrThree = pcf->PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name);
