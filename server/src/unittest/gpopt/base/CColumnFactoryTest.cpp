...
CColumnFactoryTest::EresUnittest_Basic()
{
	...
	// typed colref
	CColRef *pcrOne = cf.PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier);
	GPOS_ASSERT(pcrOne == cf.PcrLookup(pcrOne->m_ulId));
	cf.Destroy(pcrOne);

	// typed/named colref
	CWStringConst strName(GPOS_WSZ_LIT("C_CustKey"));
	CColRef *pcrTwo = cf.PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, CName(&strName));
