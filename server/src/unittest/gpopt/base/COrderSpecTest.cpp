...
COrderSpecTest::EresUnittest_Basics()
{
	...
	CColRef *pcr1 = pcf->PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name);
	CColRef *pcr2 = pcf->PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name);
	CColRef *pcr3 = pcf->PcrCreate(pmdtypeint4, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name);
