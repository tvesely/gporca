...
CConstraintTest::EresUnittest_NegativeTests()
{
	...
	CName name(pmp, &str);
	CAutoP<CColRef> pcr(COptCtxt::PoctxtFromTLS()->Pcf()->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(100), IDefaultTypeModifier, name));
...
CConstraintTest::EresUnittest_ConstraintsOnDates()
{
	...
	CName name(pmp, &str);
	CAutoP<CColRef> pcr(COptCtxt::PoctxtFromTLS()->Pcf()->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier, name));
