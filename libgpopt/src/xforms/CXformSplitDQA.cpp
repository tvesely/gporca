	...
	)
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		if (popScAggFunc->FDistinct())
		{
			...
			CColRef *pcrLocal = pcf->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier);
	...
	)
{
	...
	else
	{
		...
		pcrSecondStage = pcf->PcrCreate(pmdtype, GPOS_NEW(pmp) CMDIdGPDB(0), IDefaultTypeModifier);
	}
	...
	)
{
	...
	CColRef *pcrAdditionalGrpCol = pcf->PcrCreate(pmdtype, popScalar->PmdidCollation(), popScalar->ITypeModifier());
