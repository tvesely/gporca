	...
	)
{
	...
	else
	{
		...
		pcrCount = pcf->PcrCreate(pmdtypeCount, popCount->PmdidCollation(), popCount->ITypeModifier());
		...
		pcrSum = pcf->PcrCreate(pmdtypeSum, popSum->PmdidCollation(), popSum->ITypeModifier());
